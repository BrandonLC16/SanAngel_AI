...
def test_environment_example_keeps_all_secrets_empty() -> None:
    ...
    assert values["ASSISTANT_BRANCH_CODE"] == ""
...
def test_gitignore_protects_local_environment_without_hiding_example() -> None:
    ...
    assert "*.assistant-profile.json" in ignore_rules
...
def test_all_branch_installations_share_one_generic_system_prompt() -> None:
    prompt_directory = REPOSITORY_ROOT / "backend" / "app" / "prompts"
    prompt_files = {path.name for path in prompt_directory.glob("*.txt")}
    prompt = (prompt_directory / "base_system_prompt.txt").read_text(encoding="utf-8")

    assert prompt_files == {"base_system_prompt.txt"}
    assert "ASSISTANT_BRANCH_CODE" not in prompt
    assert "sucursal-demo" not in prompt
    assert "sucursal asignada" in prompt
